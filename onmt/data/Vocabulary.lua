...
function Vocabulary.init(name, dataFile, vocabFile, vocabSize, wordsMinFrequency, featuresVocabsFiles, validFunc, keepFrequency, idxFile)
  ...
  if vocabFile:len() > 0 then
    ...
    loadVocab = onmt.utils.Dict.new()
    loadVocab:loadFile(vocabFile)
    _G.logger:info(' * Loaded ' .. loadVocab:size() .. ' ' .. name .. ' words')
  end
  ...
  if wordVocab == nil or keepFrequency or (#featuresVocabs == 0 and numFeatures > 0) then
    ...
    if vocabFile:len() > 0 then
      ...
      _G.logger:info(' * Compact ' .. genWordVocab:getRealsize() .. ' ' .. name .. ' words')
    else
      genWordVocab, genFeaturesVocabs = Vocabulary.make(dataFile, validFunc, idxFile)
    end
    ...
    if wordVocab == nil then
      if wordsMinFrequency[1] > 0 then
        if vocabFile:len() > 0 then
          ...
          wordVocab = genWordVocab:pruneByMinFrequency(wordsMinFrequency[1], false)
        end
      elseif vocabSize[1] > 0 then
        if vocabFile:len() > 0 then
          wordVocab = genWordVocab:prune(vocabSize[1], true)
        else
          wordVocab = genWordVocab:prune(vocabSize[1], false)
        end
      else
      ...
      _G.logger:info(' * Created word dictionary of size '
                       .. wordVocab:getRealsize() .. ' (pruned from ' .. originalSizes[1] .. ')')
